...
function displayChat( content ) {
  ...
  var filterBtn = "<div class='btn-group' id='filterBtn'>" +
  ...
  collapseBtn.innerHTML = "<i class='fa fa-chevron-down'></i></button>";
  ...
  var messageCount   = [ countMessages( content[ 0 ].date ), countMessages( content[ 1 ].date ) ];
...
function filterUsers( structArray ) {
  ...
  for ( var j = 0; j < structArray.date.length; j++ ) {
    var testMatch = reDate.exec( structArray.date[ j ] );
  ...
  for ( j = 0; j < uniqueNames.length; j++ ) {
    ...
    for ( var i = 0; i < structArray.date.length; i++ ) {
      if ( structArray.name[ i ] == name ) {
        ...
        var timeMatch = reTime.exec( structArray.time[ i ] );
        var dateMatch = reDate.exec( structArray.date[ i ] );
